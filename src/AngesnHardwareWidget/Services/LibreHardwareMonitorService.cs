    ...
    private double? Temperature(ISensor? sensor)
    {
        ...
        if (!_warnedAboutZeroTemperature)
        {
            ...
            AppLog.Warn(
                ...
                "unavailable. For CPU temperature this usually means LibreHardwareMonitor could " +
                "not access its low-level hardware driver. Verify that PawnIO is installed and " +
                "running, and that the widget is elevated.");
        }
